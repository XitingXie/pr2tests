...
def _render_trace_link(trace_html_path: str) -> str:
    """Render a link to the LLM trace log if available."""
    if not trace_html_path:
        return ""
    return """\
<h2>LLM Trace Log</h2>
<div class="trigger-info">
    <p><a href="trace.html" target="_blank">View LLM Trace Log</a> — detailed log of LLM interactions during test execution.</p>
</div>
"""
...
{_render_trace_link(report.trace_html_path)}
<footer>
...
def write_report_html(report: ReportData, output_dir: Path) -> Path:
    ...
    # Copy trace.html into the report output dir if available
    if report.trace_html_path:
        trace_src = Path(report.trace_html_path)
        if trace_src.exists():
            shutil.copy2(trace_src, output_dir / "trace.html")
